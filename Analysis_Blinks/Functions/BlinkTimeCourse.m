...
for task = 1:2
    ...
    % For doing plots related to overall head movement characteristics: 
    Cut = median(metrics.BlinkRate(:,task));
    HighBlink = find(metrics.BlinkRate(:,task) > Cut);
    LowBlink = find(metrics.BlinkRate(:,task) < Cut); 

    Cut = median(metrics.HeadPeakVelocity(:,task));
    HighMove = find(metrics.HeadPeakVelocity(:,task) > Cut);
    LowMove = find(metrics.HeadPeakVelocity(:,task) < Cut); 
...
%     shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibitionVOR(SelectSub,task,:))),2*std(squeeze(metrics.BlinkInhibitionEye(SelectSub,task,:)))/sqrt(length(SelectSub)),'lineProps',{'-','Color',[0.5 0.5 0.5]})
    ...
    % For Subjects Depending on the Group they are in 


    figure(1+task)
    hold on;
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibition(HighBlink,task,:))),2*std(squeeze(metrics.BlinkInhibition(HighBlink,task,:)))/sqrt(length(HighBlink)),'lineProps',{'-','Color',ColorHead})
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibitionEye(HighBlink,task,:))),2*std(squeeze(metrics.BlinkInhibitionEye(HighBlink,task,:)))/sqrt(length(HighBlink)),'lineProps',{'-','Color',ColorSacc})
    xlabel('Time from Movement Onset [ms]')
    ylabel('Probability of Blink')
    xlim([-1000 700])
    ylim([-0.04 0.4])
    plot([0 0],[0 0.5],'k--')
    plot([-1000 700],[0 0],'k-')
    plot([0 mean(metrics.SaccDuration(HighBlink,task))],[-0.01 -0.01],'-','LineWidth',2,'Color',ColorSacc)
    plot([0 mean(metrics.HeadDuration(HighBlink,task))],[-0.03 -0.03],'-','LineWidth',2,'Color',ColorHead)
    title('High Blink')


    figure(3+task)
    hold on;
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibition(LowBlink,task,:))),2*std(squeeze(metrics.BlinkInhibition(LowBlink,task,:)))/sqrt(length(LowBlink)),'lineProps',{'-','Color',ColorHead})
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibitionEye(LowBlink,task,:))),2*std(squeeze(metrics.BlinkInhibitionEye(LowBlink,task,:)))/sqrt(length(LowBlink)),'lineProps',{'-','Color',ColorSacc})
    xlabel('Time from Movement Onset [ms]')
    ylabel('Probability of Blink')
    xlim([-1000 700])
    ylim([-0.04 0.4])
    plot([0 0],[0 0.5],'k--')
    plot([-1000 700],[0 0],'k-')
    plot([0 mean(metrics.SaccDuration(LowBlink,task))],[-0.01 -0.01],'-','LineWidth',2,'Color',ColorSacc)
    plot([0 mean(metrics.HeadDuration(LowBlink,task))],[-0.03 -0.03],'-','LineWidth',2,'Color',ColorHead)
title('Low Blink')
    ...
    figure(5+task)
    hold on;
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibition(HighMove,task,:))),2*std(squeeze(metrics.BlinkInhibition(HighMove,task,:)))/sqrt(length(HighMove)),'lineProps',{'-','Color',ColorHead})
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibitionEye(HighMove,task,:))),2*std(squeeze(metrics.BlinkInhibitionEye(HighMove,task,:)))/sqrt(length(HighMove)),'lineProps',{'-','Color',ColorSacc})
    xlabel('Time from Movement Onset [ms]')
    ylabel('Probability of Blink')
    xlim([-1000 700])
    ylim([-0.04 0.4])
    plot([0 0],[0 0.5],'k--')
    plot([-1000 700],[0 0],'k-')
    plot([0 mean(metrics.SaccDuration(HighMove,task))],[-0.01 -0.01],'-','LineWidth',2,'Color',ColorSacc)
    plot([0 mean(metrics.HeadDuration(HighMove,task))],[-0.03 -0.03],'-','LineWidth',2,'Color',ColorHead)
    title('High Move')


    figure(7+task)
    hold on;
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibition(LowMove,task,:))),2*std(squeeze(metrics.BlinkInhibition(LowMove,task,:)))/sqrt(length(LowMove)),'lineProps',{'-','Color',ColorHead})
    shadedErrorBar(TimeVek,mean(squeeze(metrics.BlinkInhibitionEye(LowMove,task,:))),2*std(squeeze(metrics.BlinkInhibitionEye(LowMove,task,:)))/sqrt(length(LowMove)),'lineProps',{'-','Color',ColorSacc})
    xlabel('Time from Movement Onset [ms]')
    ylabel('Probability of Blink')
    xlim([-1000 700])
    ylim([-0.04 0.4])
    plot([0 0],[0 0.5],'k--')
    plot([-1000 700],[0 0],'k-')
    plot([0 mean(metrics.SaccDuration(LowMove,task))],[-0.01 -0.01],'-','LineWidth',2,'Color',ColorSacc)
    plot([0 mean(metrics.HeadDuration(LowMove,task))],[-0.03 -0.03],'-','LineWidth',2,'Color',ColorHead)
title('Low Move')
...
[x p]=partialcorr(metrics.BlinkRate(SelectSub,1,:),metrics.ModulationAmpRatio(:,1),metrics.HeadRate(SelectSub,1,:),'rows','complete');
disp(['Partial Correlation between Blink Rate and Modulation Amplitude for Blocks: r = ',num2str(x),', p = ',num2str(p) ])
...
[x p]=partialcorr(metrics.BlinkRate(SelectSub,2,:),metrics.ModulationAmpRatio(:,2),metrics.HeadRate(SelectSub,2,:),'rows','complete');
disp(['Partial Correlation between Blink Rate and Modulation Amplitude for Paint: r = ',num2str(x),', p = ',num2str(p) ])


% Also look at the correlation between blink rate and onset of the
% reduction
figure;
PlotCorrelation(metrics.BlinkRate(SelectSub,1)*60,metrics.OnsetDecrease(:,1),'Blink Rate [Blink/min]','Modulation Amplitude',0)
[x p] = corr(metrics.BlinkRate(SelectSub,1,:)*60,metrics.OnsetDecrease(:,1),'type','Spearman','rows','complete');

figure;
PlotCorrelation(metrics.BlinkRate(SelectSub,1)*60,metrics.OnsetDecrease(:,1),'Blink Rate [Blink/min]','Modulation Amplitude',0)
[x p] = corr(metrics.BlinkRate(SelectSub,2,:)*60,metrics.OnsetDecrease(:,2),'type','Spearman','rows','complete');
