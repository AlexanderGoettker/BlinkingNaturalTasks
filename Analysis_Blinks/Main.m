...
for subject = 1:length(SubjectCodes_All) % Loop through all of the subjects
    ...
    if exist([datapath,RecordingId]) > 0
        ...
        for task = 1:length(Tasks)
            ...
            check = find(Head.HeadAmplitude_deg_> Head.AmplitudeGaze_deg_);
            sub.HeadVOR{subject,task} = Head(check,:);   
            ...
            metrics.HeadPeakVelocity(subject,task) = nanmean(Head.HeadPeakVel_deg_s_);

            metrics.HeadDuration(subject,task) = mean(Head.HeadDuration_ms_);
            metrics.HeadVORDuration(subject,task) = mean(sub.HeadVOR{subject,task}.HeadDuration_ms_);
            ...
            for aa = 1:length(Intervalls)-1 % loop throug the intervals
                ...
                % Do a median split based on the head movement contribution
                % to the gaze shift 
                Cut = median(Head.HeadAmplitude_deg_(comb));
                check_low = find(Head.HeadAmplitude_deg_(comb) <= Cut);
                check_high = find(Head.HeadAmplitude_deg_(comb) > Cut);

                metrics.HeadAmpLOW(subject,task,aa) = mean(Head.HeadAmplitude_deg_(comb(check_high)));
                metrics.HeadVelPrctLOW(subject,task,aa) = mean(Head.HeadPeakVel_deg_s_(comb(check_low)));
                metrics.PropBlinkPrctLOW(subject,task,aa) = mean(Head.BlinkPresent(comb(check_low)));
                metrics.HeadAmpPrctLOW(subject,task,aa) = nanmean(Head.AmplitudeGaze_deg_(comb(check_low)));
                metrics.NumTrialsPrcLOW(subject,task,aa) = length(comb(check_low));

                metrics.HeadAmpHIGH(subject,task,aa) = mean(Head.HeadAmplitude_deg_(comb(check_high)));
                metrics.HeadVelPrctHIGH(subject,task,aa) = mean(Head.HeadPeakVel_deg_s_(comb(check_high)));
                metrics.PropBlinkPrctHIGH(subject,task,aa) = mean(Head.BlinkPresent(comb(check_high)));
                metrics.HeadAmpPrctHIGH(subject,task,aa) = nanmean(Head.AmplitudeGaze_deg_(comb(check_high)));
                metrics.NumTrialsPrcHIGH(subject,task,aa) = length(comb(check_high));  
            end
            
            % Do the same based on Gaze Amplitude
            Intervalls = [0:5:100]; % Define intervals in prctile space
            for aa = 1:length(Intervalls)-1 % loop throug the intervals
                % find the relevant movements
                comb = find(Head.AmplitudeGaze_deg_ > prctile(Head.AmplitudeGaze_deg_,Intervalls(aa)) & Head.AmplitudeGaze_deg_ < prctile(Head.AmplitudeGaze_deg_,Intervalls(aa+1)));
                % Compute the statistics for this selection
                metrics.AMPHeadVelPrct(subject,task,aa) = mean(Head.HeadPeakVel_deg_s_(comb));
                metrics.AMPPropBlinkPrct(subject,task,aa) = mean(Head.BlinkPresent(comb));
                metrics.AMPHeadDurationPrct(subject,task,aa) = mean(Head.HeadDuration_ms_(comb));
                metrics.AMPHeadAmpPrct(subject,task,aa) = nanmean(Head.AmplitudeGaze_deg_(comb));
                metrics.AMPNumTrialsPrc(subject,task,aa) = length(comb);
                ...
                metrics.AMPHeadAmpLOW(subject,task,aa) = mean(Head.HeadAmplitude_deg_(comb(check_high)));
                metrics.AMPHeadVelPrctLOW(subject,task,aa) = mean(Head.HeadPeakVel_deg_s_(comb(check_low)));
                metrics.AMPPropBlinkPrctLOW(subject,task,aa) = mean(Head.BlinkPresent(comb(check_low)));
                metrics.AMPHeadAmpPrctLOW(subject,task,aa) = nanmean(Head.AmplitudeGaze_deg_(comb(check_low)));
                metrics.AMPNumTrialsPrcLOW(subject,task,aa) = length(comb(check_low));

                metrics.AMPHeadAmpHIGH(subject,task,aa) = mean(Head.HeadAmplitude_deg_(comb(check_high)));
                metrics.AMPHeadVelPrctHIGH(subject,task,aa) = mean(Head.HeadPeakVel_deg_s_(comb(check_high)));
                metrics.AMPPropBlinkPrctHIGH(subject,task,aa) = mean(Head.BlinkPresent(comb(check_high)));
                metrics.AMPHeadAmpPrctHIGH(subject,task,aa) = nanmean(Head.AmplitudeGaze_deg_(comb(check_high)));
                metrics.AMPNumTrialsPrcHIGH(subject,task,aa) = length(comb(check_high));            
            end
            ...
            for VOR_mov = 1:height(sub.HeadVOR{subject,task} )
                Start = sub.HeadVOR{subject,task}.Onset_ms_(VOR_mov)-Onset;
                if Start > 1001 & Start+1000 <length(BlinkVek)
                    BlinkFreqVOR(VOR_mov,:) = BlinkVek(round(Start+TimeVek));
                else
                    BlinkFreqVOR(VOR_mov,:) = NaN(length(TimeVek),1);
                end
            end

            % Collect the data
            metrics.BlinkInhibitionEye(subject,task,:)= movmean(nanmean(BlinkFreqEye),5);
            metrics.BlinkInhibitionVOR(subject,task,:)= movmean(nanmean(BlinkFreqVOR),5);
...
% Use a logistic regression to compare whether head velocity or gaze
% amplitude is the critical factor

[metrics]=LogisticRegression(metrics,sub,SelectSub);

[metrics]=ComputePsychometric(metrics,sub,SelectSub);

[metrics]=ComputePsychometric_Position(metrics,sub,SelectSub);

% Look at Correlations with the Blinkometric Curves
ComputeCorrelationBlinkometric(metrics,SelectSub)

ComputeCorrelationBlinkometric_Position(metrics,SelectSub)
